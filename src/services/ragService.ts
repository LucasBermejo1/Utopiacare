...
export function formatRAGContextForPrompt(context: RAGContext): string {
  ...
  prompt += "🚨🚨🚨🚨🚨 REGLA ABSOLUTA PARA SALUDOS SIMPLES (LEER PRIMERO) 🚨🚨🚨🚨🚨:\n";
  prompt += "Si el usuario dice SOLO 'Hola', 'Hola!', 'Buenos días', 'Buenas', o cualquier saludo simple SIN MÁS TEXTO:\n";
  prompt += "✅ Responde ÚNICAMENTE con: '¡Hola! ¿En qué puedo ayudarte?' o similar (máximo 1-2 frases)\n";
  prompt += "❌❌❌ PROHIBIDO ABSOLUTO en saludos simples:\n";
  prompt += "  - NO menciones productos\n";
  prompt += "  - NO des recomendaciones\n";
  prompt += "  - NO describas el perfil del usuario\n";
  prompt += "  - NO des información adicional\n";
  prompt += "  - NO menciones ingredientes\n";
  prompt += "  - NO menciones rutinas\n";
  prompt += "  - NO menciones nada más que el saludo y preguntar en qué puedes ayudar\n";
  prompt += "⚠️⚠️⚠️ Si el mensaje del usuario es SOLO un saludo, tu respuesta DEBE ser SOLO un saludo breve. NADA MÁS.\n\n";
